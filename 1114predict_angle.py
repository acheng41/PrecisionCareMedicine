...
from pre import moving_average_filter, butterworth_lowpass_filter
...
for n in d:
    ...
    joint = gait_aligned_jnt(
        g, jnt_angles_all_l, jnt_angles_all_r, jnt_pos_all_l, jnt_pos_all_r, t_trackers
    )
...
X_train, X_test, y_train, y_test = train_test_split(
    feature_matrix, ankle_matrix, test_size=0.2, random_state=42
)
...
print(
    f"Random Forest - MSE: {mse_rf_mean:.4f} ± {mse_rf_std:.4f}, R²: {r2_rf_mean:.4f} ± {r2_rf_std:.4f}"
)
print(
    f"Linear Regression - MSE: {mse_lr_mean:.4f} ± {mse_lr_std:.4f}, R²: {r2_lr_mean:.4f} ± {r2_lr_std:.4f}"
)
print(
    f"KNN - MSE: {mse_knn_mean:.4f} ± {mse_knn_std:.4f}, R²: {r2_knn_mean:.4f} ± {r2_knn_std:.4f}"
)
...
plt.plot(
    [y_true.min(), y_true.max()], [y_true.min(), y_true.max()], "k--", lw=2
)  # 理想的对角线
...
cv_results = cross_validate(
    rf, X_train, y_train, cv=5, return_estimator=True, scoring="neg_mean_squared_error"
)
# feature importance
...
plt.barh(
    np.arange(len(avg_feature_importance)),
