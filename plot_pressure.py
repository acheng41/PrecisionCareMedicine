...
# COP
for i in range(50):
    ax_l.plot(gait['cop_y_l'][gait['strike_l'][20+i]:gait['off_l'][20+i]],
              gait['cop_x_l'][gait['strike_l'][20+i]:gait['off_l'][20+i]])
    ax_r.plot(gait['cop_y_r'][gait['strike_r'][20 + i]:gait['off_r'][20 + i]],
...
for n,i in enumerate([1,2,4,7,8,9]):
    ...
    for j in range(8):
        gait = get_gait_parameters_insole(insoleAll_r[j*t_split:(j+1)*t_split,:], insoleAll_l[j*t_split:t_split*(j+1),:],
                                                t_insole_r[j*t_split:t_split*(j+1),:], t_insole_l[j*t_split:t_split*(j+1),:])
